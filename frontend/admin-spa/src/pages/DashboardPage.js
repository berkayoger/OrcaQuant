...
export default function DashboardPage() {
    ...
    const { data: chartData } = useQuery({
        ...
        queryFn: async () => (await api.get('/admin/analytics/user-growth')).data,
        staleTime: 60000
    });
    const { data: activities } = useQuery({
        queryKey: ['recent-activities'],
        queryFn: async () => (await api.get('/admin/activities/recent')).data,
    ...
    return (<div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard title="Toplam Gelir" value={(metrics?.revenue.total ?? 0).toLocaleString('en-US', {
            style: 'currency',
            currency: 'USD'
        })}/>
      ...
      <div className="card">
        ...
        <div className="space-y-2 max-h-64 overflow-y-auto">
          {(activities || []).map((activity) => (<div key={activity.id} className="flex items-center gap-3 p-2 rounded bg-neutral-800/50">
          ...
          {(!activities || activities.length === 0) && (<div className="text-sm text-neutral-400">Kayıt bulunamadı.</div>)}
        </div>
      </div>
    </div>);
}
...
function SystemMetric({ label, value, unit }) {
