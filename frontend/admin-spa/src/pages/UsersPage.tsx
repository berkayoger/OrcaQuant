...
type User = { id: number; email: string; plan: string; status: string; created_at: string }

type UsersResponse = { items: User[]; total: number }

type FilterPayload = { from?: string; to?: string; status?: string; plan?: string }
...
export default function UsersPage() {
  ...
  const columns = useMemo<ColumnDef<User>[]>(
    () => [
      ...
      },
      {
        ...
        header: 'Kayıt Tarihi',
        cell: ({ getValue }) => new Date(getValue<string>()).toLocaleString('tr-TR')
      }
    ],
    []
  )
  ...
  function handleCsvExport() {
    ...
    for (const user of items) {
      csvRows.push(
        [user.id, user.email, user.plan, user.status, user.created_at.replace('T', ' ')].join(',')
      )
    }
  ...
  function handleFiltersChange(filters: FilterPayload) {
