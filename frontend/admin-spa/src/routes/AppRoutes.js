...
function NavLink({ to, children }) {
    return (<RRNavLink to={to} end className={({ isActive }) => `block px-3 py-2 rounded-md text-sm font-medium ${isActive ? 'bg-neutral-800 text-white' : 'text-neutral-400 hover:text-white hover:bg-neutral-900'}`}>
      {children}
    </RRNavLink>);
}
